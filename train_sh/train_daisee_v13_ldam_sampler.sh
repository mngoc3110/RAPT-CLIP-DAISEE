...
# =============================================================================
# DAiSEE v13 — LDAM + WeightedRandomSampler for balanced batches
# Key change from v12:
#   --use-weighted-sampler: ensures each batch has ~equal class representation
#   Without sampler: class 0 (247/5358=4.6%) → only 0.37 samples per batch
#   With sampler: each class gets ~2.7 samples per batch → model learns all 3
#   Combined with LDAM margins → strong class separation from epoch 0
# =============================================================================
...
echo "Starting DAiSEE v13 (LDAM + WeightedSampler) Training..."
...
python3 main.py \
  --mode train \
  --exper-name DAiSEE_3class_v13_ldam_sampler \
  --dataset DAiSEE \
  --gpu 0 \
  --epochs 30 \
  --batch-size 8 \
